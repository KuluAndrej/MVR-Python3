...
def sinh_(w0, w1, x):
    ...
    return np.sinh(x * w1 + w0)

def cosh_(w0, w1, x):
    cosh_.NumParam = 2
    cosh_.NumVars = 1

    return np.cosh(x * w1 + w0)

def tanh_(w0, w1, x):
    tanh_.NumParam = 2
    tanh_.NumVars = 1

    return np.tanh(x * w1 + w0)

def arcsinh_(w0, w1, x):
    arcsinh_.NumParam = 2
    arcsinh_.NumVars = 1

    return np.arcsinh(x * w1 + w0)

def arccosh_(w0, w1, x):
    arccosh_.NumParam = 2
    arccosh_.NumVars = 1

    return np.arccosh(x * w1 + w0)

def arctanh_(w0, w1, x):
    arctanh_.NumParam = 2
    arctanh_.NumVars = 1

    return np.arctanh(x * w1 + w0)

def bessel_(w0, w1, x):
    bessel_.NumParam = 2
    bessel_.NumVars = 1

    return np.i0(x * w1 + w0)

def sinc_(w0, w1, x):
    sinc_.NumParam = 2
    sinc_.NumVars = 1

    return np.sinc(x * w1 + w0)
