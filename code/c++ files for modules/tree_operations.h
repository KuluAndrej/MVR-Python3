...
int find_subtrees_sizes(vector<vector<int> >& matr, vector<int>& subtrees_sizes, int root) {  
  subtrees_sizes[root] = 1;
  for (int i = 0; i < matr[root].size(); ++i) {
    subtrees_sizes[root] += find_subtrees_sizes(matr, subtrees_sizes, matr[root][i]);    
  }
  return subtrees_sizes[root];
}
